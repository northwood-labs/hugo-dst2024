...
export default class TlsScore extends PureComponent {
  ...
  render(props, state) {
    if (state.data) {
      if (state.data.error) {
        console.error(`TlsScore: ${state.data.error}`);
            ...
            <tr>
              ...
              <td class="ui-tbody-td-value">
                {state.data.tlsVersions.tls13 ? (
                  <SvgCheck className="ui-badge-wrap ui-badge-success-wrap animate-flip-in-x" label="TLS 1.3 enabled" />
                ) : (
                  <SvgX className="ui-badge-wrap ui-badge-error-wrap animate-flip-in-x" label="TLS 1.3 disabled" />
                )}

                {state.data.tlsVersions.tls12 ? (
                  <SvgCheck className="ui-badge-wrap ui-badge-success-wrap animate-flip-in-x" label="TLS 1.2 enabled" />
                ) : (
                  <SvgX className="ui-badge-wrap ui-badge-error-wrap animate-flip-in-x" label="TLS 1.2 disabled" />
                )}
              </td>
            </tr>
            <tr>
              ...
              <>
                ...
                {conn.cipherSuites.map(suite => (
                  <>
                    <tr>
                      <td class="ui-tbody-td-key">
                        <span class="inline-block pl-2 sm:pl-4"><code class="not-prose code">{suite.ianaName}</code></span>
                      </td>
                      <td class="ui-tbody-td-value">
